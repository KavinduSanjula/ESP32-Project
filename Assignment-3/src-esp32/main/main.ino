...
const char* tcphost = "192.168.43.236";
int tcpport = 4444;
...
void loop() {
  ...
  if (client.connect(tcphost,tcppost)){
    sendValuesToTcpServer();
  }
  dealy(10000);
}
...
void sendValuesToTcpServer(){
  ...
  client.print(&timeinfo,"%H");
  client.print(',');
  client.print(&timeinfo,"%M");
  client.print(',');
  client.print(&timeinfo,"%S");
  client.print(',');
